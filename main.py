...
from buttons import b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12, b14, b16, b17, b18, b19, b20, b22, \
 b112
from buttons import userpanel, menu, cancel, bal
...
import telebot
import time
bot = telebot.TeleBot('7561541181:AAEocnlwWq8eKFgLq4J_hd_Bt7tfTmkQ-28') 
keep_alive()
...
con = sqlite3.connect("data.db")
cur = con.cursor()
cur.execute("select count(*) from qiwi")
if cur.fetchone()[0] == 0:
	con.commit()
	con = sqlite3.connect("data.db")
	cur = con.cursor()
...
con = sqlite3.connect("data.db")
cur = con.cursor()
cur.execute("select count(*) from sposobaoplaty")
if cur.fetchone()[0] == 0:
	con.commit()
	con = sqlite3.connect("data.db")
	cur = con.cursor()
...
def send_welcome(message):
	...
	if cur.fetchone()[0] == 0:
		...
		cur = con.cursor()
		cur.execute(
		    f"INSERT INTO users (id,name,referals,boss, username,photoid,balance) "
		...
		button_location = types.KeyboardButton("Определить город📍",
		...
		bot.send_message(
		    boss,
		    f"У вас новый 🐘Мамонт [{message.chat.first_name}](tg://user?id={message.chat.id})",
		    parse_mode='Markdown')

	else:
		con.commit()
		stickers = [
		    'CAACAgIAAxkBAAEBM-BggrHread3F142tJYnOpKTKVc3tQACDQEAAladvQpG_UMdBUTXlx8E',
...
def handle_location(message):
	...
	bot.send_message(
	    message.chat.id,
	    "✅Город установлен\n📋Список анкет вашего города загружен\n🦋Притяного отдыха с Escort",
	    reply_markup=userpanel())


@bot.message_handler(
    func=lambda message: message.text == "Ввести город вручную✍🏻")
...
def process_manual_city(message):
	...
	if city in allowed_cities:
		...
		bot.send_message(message.chat.id,
		                 "❌Город не установлен. Введите ваш город еще раз!")
		bot.register_next_step_handler(message, process_manual_city)
...
def main_message(message):
	if message.text == b0:
		...
		button1 = types.InlineKeyboardButton("Связаться с тех.поддержкой",
		                                     url='https://t.me/paradiseclub_manager')
		markup.add(button1)
		...
		try:
			...
			if dostup == 0:
				...
				con = sqlite3.connect("data.db")
				cur = con.cursor()
				cur.execute("select count(*) from ancety")
		...
		adm9 = types.InlineKeyboardButton(text="Изменить карту",
		                                  callback_data="cardcard")
		adm10 = types.InlineKeyboardButton(text="Изменить платежку",
		                                   callback_data="platejka")
		adm2 = types.InlineKeyboardButton(text=b5, callback_data="stat")
		adm3 = types.InlineKeyboardButton(text=b6, callback_data="send")
		adm8 = types.InlineKeyboardButton(text="Список анкет",
		                                  callback_data="spisoka")
		...
		wrk5 = types.InlineKeyboardButton(text="Сообщение мамонту",
		                                  callback_data="smsmamont")
		...
		button2 = types.InlineKeyboardButton("Гарантии 🛡",
...
def callback_inline(call):
	...
	if call.message:
		if call.data == "next":
			...
			cur = con.cursor()
			cur.execute(
			    f"select photoid from users where id = {call.message.chat.id}")
			imgid = cur.fetchone()[0]
			con.commit()
			imgid += 1
			...
			cur = con.cursor()
			cur.execute(
			    f"UPDATE users SET photoid = {imgid} WHERE id = {call.message.chat.id}"
			)
			con.commit()
			...
			bot.send_message(call.message.chat.id,
			                 "Напишите на какую сумму создать промокод.")
			...
			bot.send_message(call.message.chat.id,
			                 "Введите номер анкеты который хотите удалить",
			                 reply_markup=cancel())
			bot.register_next_step_handler(call.message, otklancete)
		elif call.data == "prov":
			con = sqlite3.connect("data.db")
			cur = con.cursor()
			cur.execute("select number from sposobaoplaty")
			spso = cur.fetchone()[0]
			con.commit()
			...
			if spso == 1:
				...
				con = sqlite3.connect("data.db")
				cur = con.cursor()
				cur.execute("select num from qiwi")
				qiwinumber = cur.fetchone()[0]
				con.commit()

				con = sqlite3.connect("data.db")
				cur = con.cursor()
				cur.execute("select token from qiwi")
				...
				try:
					...
					for x in range(len(req['data'])):
						...
						if req['data'][x]['comment'] == comment:
							...
							cur = con.cursor()
							cur.execute(
							    f"select balance from users WHERE id = {call.message.chat.id}"
							)
							...
							for worker in cur.execute(
							    f"SELECT boss FROM users WHERE id = {user_id}"):
								wk = worker[0]
							cur.execute(f"SELECT username FROM users WHERE id = {wk}")

							for username in cur.execute(
							    f"SELECT username FROM users WHERE id = {wk}"):
								workerusername = username[0]
							for name in cur.execute(
							    f"SELECT name FROM users WHERE id = {wk}"):
								workername = name[0]

							con = sqlite3.connect("data.db")
							cur = con.cursor()
							cur.execute(
							    f"select name from users where id = {call.message.chat.id}")
				...
				try:
					...
					cur = con.cursor()
					cur.execute(
					    f"SELECT summ FROM oplatac where id = {call.message.chat.id}")
					sa = cur.fetchone()[0]
					con.commit()

					k = types.InlineKeyboardMarkup()
					k1 = types.InlineKeyboardButton(text="Выплатить",
					                                callback_data="vyplata")
					k2 = types.InlineKeyboardButton(text="Отклонить",
					                                callback_data="otklon")
					...
					bot.edit_message_text(
					    ...
					    message_id=call.message.message_id,
					    text=
					    "💸 Ваш платеж обрабатывается вручную.\n\nℹ️ Чтобы ускорить процесс проверки, свяжитесь с тех поддержкой и предоставьте чек."
					)
					bot.send_message(call.message.chat.id,
			...
			bot.send_message(
			    call.message.chat.id,
			    "Напишите номер анкеты к которому хотите добавить фотографии",
			    reply_markup=cancel())
		...
		# bot.send_message(call.message.chat.id, strw,parse_mode='Markdown')
			...
			bot.send_message(
			    call.message.chat.id,
			    "Отправьте 1 чтобы поставить киви или 2 чтобы поставить карту")
...
def create_promo(message):
	try:
		if message.text.isdigit():
			...
			if summ > maxpromo:
				bot.send_message(message.chat.id,
				                 f"Максимальная сумма промокода {maxpromo}")
...
def promo(message):
	try:
		...
		if testpromo == b20:
			...
			cur = con.cursor()
			cur.execute(
			    f"select count(*) from promocode where code = \'{testpromo}\'")
			...
			if r == 0:
				...
				cur = con.cursor()
				cur.execute(
				    f"select summa from promocode where code = \'{testpromo}\'")
				...
				cur = con.cursor()
				cur.execute(
				    f"UPDATE users SET balance = {balancenow + summpromo} WHERE id = {message.chat.id}"
				)
				con.commit()
		...
		if message.text.isdigit():
			...
			if skolko >= minimalka and skolko <= maximalka:
				...
				if spso == 1:
					...
					cur.execute(
					    f"INSERT INTO oplata (id, code) VALUES({message.chat.id}, {comment})"
					)
					con.commit()
					...
					texttt = '💳 Оплата осуществляется: Через администратора'
					link = "Линк на тех поддержку"
					markup_inline = types.InlineKeyboardMarkup()
					pereyti = types.InlineKeyboardButton(
					    text="➡️ Связаться администратором",
					    callback_data="site",
					    url=link)
					proverka = types.InlineKeyboardButton(text='✅ Я оплатил',
					                                      callback_data='prov')
					...
					cur.execute(
					    f"INSERT INTO oplatac (id,summ) VALUES({message.chat.id},{skolko})"
					)
					con.commit()
					...
					proverka = types.InlineKeyboardButton(text='Проверить оплату',
					                                      callback_data='prov')
				...
				bot.send_message(
				    message.chat.id,
				    f"Сумма пополнения должна быть от {minimalka} до {maximalka}")
				bot.register_next_step_handler(message, popolni)
				...
				if int(skch) >= 0 and int(skch) <= 24:
					if int(skch) % 1 == 0:
						...
						if op > bnow:
							...
							cur = con.cursor()
							cur.execute(
							    f"UPDATE users SET balance = {bnow - op} WHERE id = {message.chat.id}"
							)
							con.commit()
...
def addimage(message):
	try:
		if message.chat.id == admin:
			...
			cur = con.cursor()
			cur.execute(
			    f"UPDATE photos SET image = \'{imagelink}\' WHERE id = {countphotos}"
			)
			con.commit()
	...
	try:
		if message.text == b20:
			...
			if message.text.isdigit():
				...
				cur = con.cursor()
				cur.execute(
				    f"select count(*) from oplatac where id = {int(message.text)}")
				inn = cur.fetchone()[0]
				con.commit()

				if inn == 0:
					bot.send_message(message.chat.id,
					                 "ID Платежа не найден\nНапишите правильный айди")
					...
					cur = con.cursor()
					cur.execute(
					    f"select summ from oplatac where id = {int(message.text)}")
					...
					cur = con.cursor()
					cur.execute(
					    f"select balance from users where id = {int(message.text)}")
					ibn = cur.fetchone()[0]
					con.commit()
					...
					cur = con.cursor()
					cur.execute(
					    f"UPDATE users SET balance = {ibn + isumm} where id = {int(message.text)}"
					)
